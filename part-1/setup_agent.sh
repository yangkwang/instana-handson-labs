...
function start_agent() {
  ...
  if [ "$INIT" = "systemd" ]; then
    ...
    if grep -iq 'SUSE' /etc/*release 2>/dev/null; then
      # remove SysV file on SUSE linux before enabling the service
      if [ -f /etc/init.d/instana-agent ]; then
        rm -f /etc/init.d/instana-agent
      fi
    fi
...
if [ $OPEN_J9 = "false" ]; then
  if [ $MACHINE = "s390x" ] || [ $MACHINE = "ppc64le" ]; then
    log_info "For the '${MACHINE}' architecture only OpenJ9 JDK is available. Enforcing OpenJ9 agent runtime."
    OPEN_J9="true"
  fi
fi

if [ $OPEN_J9 = "true" ]; then
